...
for i in eurostat undata ; do
    # crawl
    JAVA_OPTS="-Xmx64G -Dldfu.http.connecttimeout=60000 -Dldfu.http.sockettimeout=60000 -Dldfu.inputoriginthreads=1 -Dldfu.processingthreads=4" ldfu.sh -p http://harth.org/andreas/2017/ocdp/programs/get-${i}.n3 -p http://harth.org/andreas/2017/ocdp/programs/get-qb.n3 -p http://harth.org/andreas/2017/ocdp/programs/qb-norm.n3 -o ${date}/${i}-crawl.nq -v 2> ${date}/${i}-crawl.log
    ...
    JAVA_OPTS="-Xmx64G" ldfu.sh -p http://harth.org/andreas/2017/ocdp/programs/map-${i}.n3 -i ${date}/${i}-crawl-clean.nq.gz -o ${date}/${i}.nq -v 2> ${date}/${i}-map.log
    # parse to clean files
